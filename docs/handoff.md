...
- **A second, independent pipeline sharing the same primitives** — [`firmware/src/commands.h`](../firmware/src/commands.h) / [`commands.cpp`](../firmware/src/commands.cpp). `pollDeviceCommandsIfDue()` runs the same shape as the tap pipeline — `signDeviceMessage()` (pure, extracted alongside `signPayload()` from a shared `hmacSha256Hex()` in `crypto.cpp`) → `pollDeviceCommand()` (I/O) → `ledIndicateSessionStart()`/`ledIndicateSessionEnd()` (I/O) — but on its own ~5s interval, non-blocking, called once at the top of `loop()` so it never delays tap reads. Reuses `net.h`'s `DeviceCommand` result type rather than inventing a second error-handling convention.
